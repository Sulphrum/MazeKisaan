...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      // The local API persists demo data here. Ignore it so a successful
      // harvest does not reload React and send the farmer back to Home.
      watch: { ignored: ['**/.figma/**', '**/server/data/db.json'] },
    },
    preview: {
      host: '0.0.0.0',
      port: parseInt(process.env.PORT || '8443'),
...
/**
 * Mounts the माझे Kisan Express Backend API routes (/api/*) directly into
 * the Vite dev server middlewares.
 */
function kisanSetuBackendPlugin(): Plugin {
  return {
    name: 'kisansetu-backend-api',
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        const url = req.url || ''
